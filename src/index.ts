...
import {
  ...
  TenantKeyRoutes,
  TenantRoutes,
  TenantModelListRoutes,
  TenantModelScaleRoute,
} from './routes';
import {
  AUTH_PREFIX,
  DATE_IN_DB_PREFIX,
  ...
  TENANT_KEY_PREFIX,
  TENANT_PREFIX,
  MODE_SCALE_LIST_PREFIX,
} from './utils/key_types';
...
import {
  checkTenantVerifyUser,
  runningModelOpenAi,
} from './controllers/UtilsForOpenAiController';
...
//Home page
app.get('/', () => 'Hello from chatgpt service! v0.0.40');
...
app.group(`${prefix}/${MODEL_OPENAI_PREFIX}`, (app) =>
  app.use(ModelOpenAiRoutes)
);
app.group(`${prefix}/${MODEL_LIST_PREFIX}`, (app) =>
  app.use(TenantModelListRoutes)
);
app.group(`${prefix}/${MODE_SCALE_LIST_PREFIX}`, (app) =>
  app.use(TenantModelScaleRoute)
);
